...
define gui.about = _p("""
Большое спасибо за прохождение "Интересный Иноземец"!

Эта игра была разработана для конкурса "Большая перемена" командой "OLEGOFRIENDER production" совместно с "Kasar' & Chursin tehnologies".

При создании игры использовались материалы из мультсериала "Смешарики" (почти все фоны и спрайты), сериала "Doctor Who" (спрайты ИИ) и рисунок А. В. Зарубина (тот, что с ракетой).

Все было сделано благодаря сервису "КиноПоиск", функции PrintScrin, программе "Paint 3D" и движку "Ren'Py".

Эта игра не является коммерческим проектом, так что мы надеемся (очень надеемся), что нас побьют из-за авторских прав.

Еще раз покорнейше благодарим за потраченное на эту игру личное время!
""")
...
define build.name = "BolshayPeremena"
...
define config.main_menu_music = "Главное меню.mp3"
...
define config.save_directory = "BolshayPeremena-1595325741"
